...
/**
 * HistoryView — Recent sessions management
 * Route: /history
 */
...
import {
  ArrowLeft, FileText, FolderTree, Table, Binary, Image,
  Play, Trash2, Bookmark, BookmarkCheck, Clock, ChevronLeft, ChevronRight
...
function kindTheme(kind: SessionKind) {
  if (kind === 'folder_diff') return 'primary'
  if (kind === 'text_diff') return 'secondary'
  return 'default'
}
...
function truncatePath(p: string, max = 48) {
  ...
  const parts = p.split('/')
  if (parts.length <= 2) return p.slice(0, max - 2) + '…'
...
async function openSession(s: Session) {
  try {
    // Re-save session to update last_opened
    await saveSession({
      id: s.id,
      kind: s.kind,
      name: s.name ?? truncatePath(s.left_path),
...
<template>
  <div class="history-view flex flex-col h-full overflow-hidden">
    ...
    <div class="history-header flex items-center gap-3 px-6 py-4 flex-shrink-0">
      <button class="btn btn-icon" @click="router.push('/')" title="返回主页">
        <ArrowLeft :size="18" />
      </button>
      <div>
        <h1 class="title">历史记录</h1>
        <p class="subtitle">{{ sessions.length }} 个会话 · 最近使用的排在最前</p>
      </div>
      <div class="flex-1" />
      <button v-if="sessions.length" class="btn btn-ghost text-xs" @click="clearAllHistory">
        <Trash2 :size="14" /> 清空全部
      </button>
    </div>
    ...
    <div v-if="error" class="error-banner flex items-center gap-2 px-6">
      <span>{{ error }}</span>
      <button class="btn btn-ghost text-xs" @click="loadSessions">重试</button>
    </div>
    ...
    <div v-else-if="!sessions.length" class="empty-state flex-1 flex flex-col items-center justify-center gap-4">
      ...
      <div class="text-center">
        <p class="text-muted text-lg mb-1">暂无历史记录</p>
        <p class="text-muted text-sm">您对比过的文件将显示在这里</p>
      </div>
    ...
    <div v-else class="session-list flex-1 overflow-y-auto px-6 pb-6">
      ...
      >
        <!-- Icon + Kind -->
        <div class="session-icon" :class="`theme-${kindTheme(s.kind)}`">
          <component :is="kindIcon(s.kind)" :size="18" />
        </div>

        <!-- Info -->
        <div class="session-info flex-1 min-w-0">
          <div class="session-kind">{{ kindLabel(s.kind) }}</div>
        ...
        <div class="session-actions flex items-center gap-2">
          <button
            class="btn btn-icon action-open"
            title="重新打开此会话"
            @click="openSession(s)"
          >
            <Play :size="16" />
          ...
          <button
            ...
            :title="confirmDelete === s.id ? '再次点击确认删除' : '删除此记录'"
...
.session-icon.theme-primary { background: rgba(37,99,235,.1); color: var(--color-accent); }
.session-icon.theme-secondary { background: rgba(22,163,74,.1); color: var(--color-green); }
.session-icon.theme-default { background: var(--color-bg3); color: var(--color-text-muted); }
...
.session-meta {
  ...
  margin-top: 4px; font-size: 11px; color: var(--color-text-muted);
}
...
.btn-ghost {
  ...
  background: transparent; color: var(--color-text-muted); cursor: pointer; transition: all 0.15s; font-size: 12px;
}
.btn-ghost:hover { border-color: var(--color-accent); color: var(--color-accent); }
.btn-primary {
  padding: 7px 16px; border-radius: 8px; border: none;
  background: var(--color-accent); color: white; cursor: pointer; font-size: 13px; font-weight: 500;
}
